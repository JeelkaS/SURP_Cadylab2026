...
class InOutManger(InOutKernel):
    ...
    def init(self, canvas:BaseCanvas):
        ...
        match canvas.modeState.get():
            ...
            case 'vmm_test': #for VMM test run
                ...
                #NOTE: No need to mess with delay time. 

                self.inputTarget    = self.processInputVMMTest #calls processInputVMMTest function within this program
                self.initNewRow     = self.init_newVMMRow #calls init_newVMMRow function within this program
                self.outputTarget   = self.processOutputVMMTest #calls processOutputVMMTest function within this program
                self.executeTarget  = self.createVMMTestPlots #calls createVMMTestPlots function within this program
                self.finishTarget   = self.createCSV #calls createCSV function within this program
...
# ----------------------------------------------------------------------------------------------------
    ...
    def processInputVMMTest(self, input:list):
        lines = None
        try:
            # Send the input command
            print(input[2])
            self.cmd.resetBuffer()
            self.cmd.write(bytearray(input[2])) # input[2] is the byteList
...
# ----------------------------------------------------------------------------------------------------
            ...
            with open(filePath, openVar, newline='', encoding='utf-8-sig') as file:
                ...
                if openVar == 'w':
                    ...
                    if self.canvas.modeState.get() == 'pulse_step_test':
                        ...
                        ('Read2' in header):
                        header.insert(3,'Read1 Ch')
                        header.insert(4,'Read1')
                    # use correct unit
                ...
                for idx in new_row:
                    ...
                    # the unit conversions are copied from convert_column in getData
                        ...
                        if ('Read1 Ch' not in new_row) and (self.results.shape[1] > 6 or \
                            self.canvas.modeState.get() == 'pulse_cycle_test'):
                            values.append('')
                            values.append('')
                        values.append(str(new_row[idx]))
                    elif self.canvas.toggledOhmsLawUnit.get() != 'uA' \
    ...
    def processOutputVMMTest(self, output):
        # Extract the channel data and current readings from the output lines
        reads = self.getValues(output[3]) # output[3] is the lines.
        ...
        new_row["Input Used"] = f"{output[2][14]:08b}" + f"{output[2][15]:08b}" + \
            f"{output[2][16]:08b}" + f"{output[2][17]:08b}" + \
            f"{output[2][18]:08b}" + f"{output[2][19]:08b}" + \
            f"{output[2][20]:08b}" + f"{output[2][21]:08b}"
            ...
            with open(filePath, openVar, newline='', encoding='utf-8-sig') as file:
                ...
                for idx in new_row:
                    ...
                    # the unit conversions are copied from convert_column in getData
                        ...
                        if self.results.empty or self.results['Input Used'].iloc[-1] != new_row[idx]:
                                values.append(str(new_row[idx]))
                    else:
                        values.append(str(new_row[idx]))
                file.write(','.join(values) + '\n')
    ...
    def createCSV(self):
        ...
        if self.canvas.csvControlVariable.get():
            ...
            match self.canvas.modeState.get():
				...
				# if VMM test is chosen, no grid csv should be made since we're reading
                # by column, not by device
    ...
    def createVMMTestPlots(self):
        if self.canvas.createSavePlots.get():
            ...
            try:
                ...
                for col, group in data.groupby(['col']):
                    column = f"({col})"
                    read1 = None
                    read2 = None

                    try:
                        # Extract Read1 and Read2 time series per column over cycles
                        if 'Read1' in group.columns:
                            read1_values = group[['cycle', 'Read1']].set_index('cycle').sort_index()
                            read1 = (f"{column}", read1_values)
                        else: # allows plots to be created even without read1
                            read1 = (f"{column}", pd.DataFrame())

                        with self.plotDataLock:
                            self.plotDataDict[column] = {'Read1': read1, 'Read2': read2}
                            self.newPlotDataEvent.set()
                            '''
                            print('testing within createPulseTestPlots')
                            print(self.newPlotDataEvent.is_set())
...
# ----------------------------------------------------------------------------------------------------
    ...
    def init_newVMMRow(self, output):
        return {"col": output[0], "cycle": output[1]}
