...
class GridWindow:
	...
	# used for VMM testing progress tracker
    def getSelectedColCount(self):
        selectedColCount = 0
        col = 0
        while col < self.columns:
            row = 0
            while row < self.rows:
                singleCellBoolean = self.gridBooleans[row][col] #boolean of the cell/button

                # if this button is not selected, continue
                if(not singleCellBoolean):
                    row += 1
                    continue
                
                # if it is selected, add to the count
                selectedColCount += 1
                # skip the rest of this column since we got it already
                row = self.rows
            col+=1
        return selectedColCount
