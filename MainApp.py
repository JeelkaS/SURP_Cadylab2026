...
class MainApp:
    ...
    def initCMD(self, baudRate = 115200, startup = False):
        try:
            ...
            time.sleep(1)
            # try two different ports, one for computer use, one for raspberry pi use
            ...
            if not cmd.isOpen():
                cmd = SerialPort('/dev/ttyUSB0', baudRate = baudRate)
                if not cmd.isOpen():
                    time.sleep(1)
    ...
    def sendButtonCallback(self):
        ...
        match self.canvas.modeState.get():
            ...
            case 'vmm_test':
                ...
                self.ui.print('Starting VMM Test ...')
                self.successfulRun = False
                self.totalDevCount = 0

                def runVMMTest(): #function to be called as a target for threading

                    #build on Quotayba's established logic above for basic Pulse Testing
                    inputByteList = self.canvas.getByteList() if self.canvas.binaryMode.get() else self.canvas.getRead2ByteList()
                    # totalDevCount will count columns instead of devices, since whole columns are sent at once
                    self.totalDevCount = self.ui.cellGrid.getSelectedColCount()*self.canvas.cycleNumber.get()
                    self.updateProgress(self.canvas)
                    self.successfulRun = self.ProcessVMMTest(inputByteList, self.canvas, devices)
                    ...
                    if self.successfulRun:
                        ...
                        self.ui.print('VMM Test Failed ...')
                        self.ui.print('aborting ...', isDone=True)
                        self.ui.after(0, self.inOutManger.hardStop)
                    
                Thread(target = runVMMTest, daemon=True).start() #peforming threading on the created function above
    ...
    def ProcessVMMTest(self, ByteList:list, canvas:BaseCanvas, devices):
        try:
            addInput = self.inOutManger.addVMMInput
            inputByteList = ByteList
            # go col by col instead of row by row
            col = 0
            while col < len(devices[0]):
                row = 0
                while row < len(devices):
                    if not self.inOutManger.isRunning():
                        return False
                    ...
                    inputByteList[22] = col  #column
                    # find all selected devices in this column
                    # convert them such that each bit in a byte represents one device,
                    # where 0 is not selected and 1 is selected, then the byte is
                    # converted to the proper ints to be sent. 8 bytes are used for this.
                    deviceBytes = [None] * 8
                    for k in range(8):
                        bitString = ''
                        j = k * 8
                        ...
                        deviceBytes[k] = bitInt
                    
                    for i in range(0,8):
                        ByteList[i+14] = deviceBytes[i]

                    if self.inOutManger.debug:
                        print(ByteList) 
                        print(f"[ {' | '.join([f'{x:02X}' for x in inputByteList])} ]")  

                    addInput(col, inputByteList.copy())
                    # skip the rest of this column since we got it already
                    row = len(devices)
                col+=1
            else:
                return True
            ...
            if self.inOutManger.newPlotDataEvent.is_set():
                ...
                for cycle, gridData in self.inOutManger.getPlotData().items():
                    ...
                    match self.canvas.modeState.get():
                        ...
                        case 'vmm_test':
                            # can only do plot for pulse test if > 1 cycles
                            if (self.canvas.cycleNumber.get() > 1):

                                #read1_data[0] is the device column string ({col})
                                if read1_data[0] not in self.deviceScatterPlots:
                                    fig, ax = plt.subplots(figsize=(10, 6))
                                    self.deviceScatterPlots[read1_data[0]] = (ax, fig)
                                    newFig = True
                                ...
                                if newFig:
                                    #for aesthetic reasons, get the column number from read1_data[0]
                                    #to use to give a different format to the plot title
                                    colVal = re.findall(r'\d+', read1_data[0])

                                    ax.set_xlabel("Cycle")
                                    if self.canvas.toggledOhmsLawUnit.get() == 'kOhm':
                                        ax.set_ylabel("Resistance (kOhm)")
                                        ax.set_title(f"Column ({colVal}) - Resistance vs. Cycle")
                                    else:
                                        ax.set_ylabel("Current (uA)")
                                        ax.set_title(f"Column ({colVal}) - Current vs. Cycle")
                                    ax.legend()
                                ...
                                if repeat and (self.inOutManger.isRunning() or self.inOutManger.newPlotDataEvent.is_set()):
                                    ...
                                    for dev, (ax, fig) in self.deviceScatterPlots.items():
                                        fig.savefig(os.path.join(plotFolder, f'Scatter_Col_{dev}.png'))
